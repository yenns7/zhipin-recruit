...
CLI 装好后还需绑定 BOSS 账号。本系统部署在云服务器，后端读不到使用者本机浏览器的登录态，因此采用**扫码登录**方式：用 BOSS 直聘 APP 扫码，纯 HTTP 流程拿到会话 Cookie（`wt2`/`wbg`/`zp_at`），即可使用全部招聘端功能。

**扫码登录操作步骤：**

1. 进入 `/boss` 页面 → 顶部「BOSS 账号管理」→ 点击「扫码登录」。
2. 用 BOSS 直聘 APP 扫描页面上的二维码 → 在手机上确认。
3. 登录成功后输入账号别名保存。系统自动加密存储会话 Cookie。

**扫码登录后可用功能（Tier-1，无需 `__zp_stoken__`）：**
- 收件箱/沟通列表、推荐候选人列表
- 查看候选人简历详情、下载简历 Markdown
- 批量导入候选人到系统候选人库
- AI 简历初筛
- 职位列表查看
- 多账号管理（切换/校验/删除）

**已下线功能（原 Tier-2，依赖短效 `__zp_stoken__`，云端无法稳定持有）：**
- 搜索候选人、打招呼、发消息、请求简历
- 通过 BOSS 系统发送面试邀请（改为系统内安排）
- 关闭/开启职位

> 安全提示：Cookie 等同 BOSS 登录态。请全程通过 HTTPS 提交；后端仅 Fernet 加密落库、不写日志。请告知试点人员用途与留存期限。
...
`/api/boss/status` 在 BOSS 账号未绑定时返回 409 `no_active_account` 属正常业务响应（去 `/boss` 页面扫码登录即可）；返回 401 `not_authenticated` 表示账号 Cookie 失效需重新扫码；只有返回 503 `boss_cli_not_installed` 才是 CLI 未就绪，需回到第 1 步。

扫码确认（`POST /api/boss/qr-login/confirm`）：200 保存成功，全功能可用。

`POST /api/boss/login/cookie` 已下线，返回 410 `gone`。
