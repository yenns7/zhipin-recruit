...
- **文档状态**：草案，待评审
- **日期**：2026-06-15
- **分支**：`feat/pipeline-stage-management`（M0 已实现部分，见下）
- **作者**：研发
...
| # | 缺口 | 角色 | 优先级 | 所属模块 |
|---|------|------|--------|----------|
| G1 | 流程阶段无法表达多轮面试（初筛/一面/二面/终面） | recruiter, interviewer | P0 | M1 |
| G2 | 阶段变更无原因/备注，不可追溯 | 全部 | P0 | M1 |
| G3 | AI 面试结果不回写流程阶段 | recruiter | P0 | M2 |
| G4 | 面试官无法录入评分/评语（scorecard） | interviewer | P0 | M2 |
| G5 | 面试记录无列表入口，报告页是孤儿 | recruiter, interviewer | P0 | M2 |
| G6 | 开放注册 + 可自选 admin 角色（安全漏洞） | admin | P0(安全) | M3 |
| G7 | 无用户/团队管理界面 | admin | P0 | M3 |
| G8 | 候选人档案不显示其在各岗位的当前阶段/时间线 | 全部 | P1 | M4 |
| G9 | 经理无法钻取单候选人旅程 | manager | P1 | M5 |
| G10 | 候选人无法在专员间转派 | manager, admin | P2 | M5 |
